...
static inline int
memmap_bin_search(uintptr_t addr)
{
...
int
memmap_test(uintptr_t start, uintptr_t end)
{
    unsigned int
        i = memmap_bin_search(start),
        j = memmap_bin_search(end - 1);
    return (i & 1) && (i == j);
}

int
memmap_enumerate(int num, uintptr_t *start, uintptr_t *end)
{
    if ((num + 1) << 1 > memmap_count) return -1;
...
int
memmap_append(uintptr_t start, uintptr_t end, int flag)
{
...
memmap_process(int verbose)
{
    ...
    for (i = 0; i != mmnode_count; ++i)
    {
        ...
        uintptr_t t;
        int f;
        t = mmnode_addr[i]; mmnode_addr[i] = mmnode_addr[j]; mmnode_addr[j] = t;
        f = mmnode_flag[i]; mmnode_flag[i] = mmnode_flag[j]; mmnode_flag[j] = f;
    }
    ...
    while (1)
    {
        if (i >= mmnode_count) break;
        
        last_vaild = v;
        // DEBUG("DEBUG: VL[%d] RL[%d] ADDR[%p] FLAG[%d]\n", vaild_level, reclaim_level, mmnode_addr[i], mmnode_flag[i]);
        while (1)
        {
            ...
            ++ i;
            if (i >= mmnode_count) break;
            if (mmnode_addr[i - 1] != mmnode_addr[i]) break;
        }
