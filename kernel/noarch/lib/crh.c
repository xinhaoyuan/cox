...
{
    int d = BIT_SEARCH_LAST(crh->base ^ node->key) >> CRH_RADIX_BITSIZE_SHIFT;
...
crh_set_base(crh_t crh, crh_key_t base)
{
    ...
    int d = BIT_SEARCH_LAST(crh->base ^ base) >> CRH_RADIX_BITSIZE_SHIFT;
...
{
    if ((intptr_t)(node->key - crh->base) <= 0) return -1;
    _crh_insert(crh, node);
    return 0;
}

crh_key_t
crh_max_step(crh_t crh)
{
    if (crh->bm == 0) return 0;
     
    int d = BIT_SEARCH_FIRST(crh->bm);
    ...
    {
        crh_key_t r_bm = (crh->heads_bm[d] >> idx) |
            (crh->heads_bm[d] << (CRH_RADIX_BITSIZE - idx));
        r = (BIT_SEARCH_FIRST(r_bm) << (d << CRH_RADIX_BITSIZE_SHIFT)) - remain;
    }
    else
    {
        crh_key_t r_bm = (crh->heads_bm[d] >> idx);
        ...
        else r = (BIT_SEARCH_FIRST(r_bm) << (d << CRH_RADIX_BITSIZE_SHIFT)) - remain;
    }
    return r;
