...
unsigned vm_x_byte(x86emu_mem_t *mem, unsigned addr)
{
  ...
  // printf("vm_x_byte: perm %02x PERM_X: %02x VALID: %02x\n", *attr, X86EMU_PERM_X, X86EMU_PERM_VALID);
...
unsigned vm_memio(x86emu_t *emu, u32 addr, u32 *val, unsigned type)
{
  ...
  /* if (type == X86EMU_MEMIO_I || */
  /* 	  type == X86EMU_MEMIO_O) */
  /* 	   ekf_kprintf("vm_mem_io: addr %08x type: %02x --- %d\n", addr, type, mem->invalid); */
  return mem->invalid;
}
