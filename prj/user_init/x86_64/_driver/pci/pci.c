...
/* #define assert(x)														\ */
/* 	 do { if (!(x)) cprintf("assertion failed: %s", #x); } while (0) */
...
static struct pci_bus pci_buses[PCI_BUSES_MAX];
int pci_buses_count;
				   ...
				   uint32_t offset)
{
	 ...
	 __outl(pci_conf1_addr_ioport, v);
}
...
{
	 pci_conf1_set_addr(f->bus->busno, f->dev, f->func, off);
	 __outl(pci_conf1_data_ioport, v);
}
...
pci_bridge_attach(struct pci_func *pcif)
{
	 ...
	 {
		  cprintf("reach the maximum number of buses.\n");
		  return 0;
	 }
	 nbus = pci_buses + (pci_buses_count ++);
	 
	 memset(nbus, 0, sizeof(struct pci_bus));
