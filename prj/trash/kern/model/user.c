...
user_thread_init_exec(proc_t proc, void *bin, size_t bin_size)
{
    ...
    if ((user_proc = kmalloc(sizeof(user_proc_s), AF_ATOMIC)) == NULL) return -E_NO_MEM;
...
{
    int ret;
    iosem_hash_head_t heads = kmalloc(sizeof(iosem_hash_head_s) * USER_IOSEM_HASH_SIZE, AF_ATOMIC);
    if (heads == NULL)
    {
        return -E_NO_MEM;
    }
...
user_thread_init(proc_t proc, uintptr_t entry, uintptr_t tls_u, size_t tls_size, uintptr_t arg0, uintptr_t arg1, uintptr_t stack_ptr)
{
    ...
    thread->io_cap      = cap;
    /* XXX no mem ? */
    thread->ioce_shadow = kmalloc(sizeof(io_ce_shadow_s) * cap, AF_ATOMIC);
