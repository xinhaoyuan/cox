...
mbox_user_send(proc_t io_proc, iobuf_index_t io_index)
{
    ...
    {
        if (user_proc_arch_mmio_open(mbox->proc, FRAME_TO_PHYS(send_io->iobuf_p),
...
mbox_user_io_attach(proc_t io_proc, iobuf_index_t io_index, mbox_t mbox, int type, size_t buf_size)
{
    ...
    else
    {
        ...
        if ((send_io->iobuf_p = frame_alloc(buf_size, AF_ATOMIC)) == NULL)
        {
            err = -E_NO_MEM;
            goto error;
        }
        
        if (user_proc_arch_mmio_open(USER_THREAD(io_proc)->user_proc, FRAME_TO_PHYS(send_io->iobuf_p),
